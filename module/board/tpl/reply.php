	...
	<?php
		...
		foreach ($cmt['data'] as $key => $value) {
			...
			echo '<a id="reply_'.$value['rp_srl'].'"'.(!empty($_DATA['rp'])&&$value['rp_srl']==$_DATA['rp'] ? ' class="active"':'').'></a>'
				...
				.'<img src="'.$_icon.'" alt="Profile" class="profile"><h5 class="author">'.$value['mb_nick'].'</h5>'
				.'<div class="reply-date"><small>'.date('Y/m/d h:m', strtotime($value['rp_update'])).'</small></div></div><div class="right"><div class="content clearfix">'.toHTML($value['rp_type'], $value['rp_content']).'</div>'
