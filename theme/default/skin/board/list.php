...
<?php if(empty($_GET['srl']) && !empty($_CFG['md_category'])){
	$cats = explode(':',(empty($_GET['cat'])?'':$_GET['cat']).':');
?>
	...
	<?php
		...
		foreach ($tmp as $val){
			$isEqual = $val == $cats[0];
			$cateurl = getUrl('','id',_MID_,'cat', urlencode($val)).($isEqual&&!$asc?'&asc':'');
...
<?php if(empty($_GET['srl']) && !empty($_CFG['md_category2'])){ ?>
	...
	<?php
		...
		foreach ($tmp as $val){
			$isEqual = $val == $cats[1];
			$cateurl = getUrl('','id',_MID_,'cat', urlencode($cats[0]).':'.urlencode($val)).($isEqual&&!$asc?'&asc':'');
			echo '<li class="d-inline mx-1"><a class="badge text-bg-secondary text-decoration-none'.($isEqual?' active" aria-current="page':'').'" href="'.$cateurl.'">'.$val.($isEqual?($asc?'▴':'▾'):'').'</a></li>';
		}
	?>
	</ol>
