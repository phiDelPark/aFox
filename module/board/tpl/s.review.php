...
<article class="clearfix" role="list">
	...
	<?php
		...
		// 확장변수가 있으면 필수만 골라냄
		if (!empty($CONFIGS['print_extra_vars'])&&!empty($_CFG['md_extra']['keys'])) {
		...
		foreach ($LIST['data'] as $key => $val) {
			...
			$box_items[false] .= '<div class="author clearfix">'.(empty($CONFIGS['review_author'])?'':'<span class="mb_nick pull-left" data-srl="'.$val['mb_srl'].'" data-rank="'.(ord($val['mb_rank']) - 48).'">'.$val['mb_nick'].'</span>').'<span class="pull-right">'.date('Y/m/d', strtotime($val['wr_update'])).'</span></div>'.$wr_extra_vars.'<div class="wr_content">'.cutstr(!$wr_permit&&$wr_secret?getLang('msg_is_secret'):escapeHtml($val['wr_content'], true, ENT_QUOTES, false), __MOBILE__?100:300).'</div>';
			?>
