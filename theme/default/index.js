...
(function($) {
	...
	$(window).load(function() {
		...
		var captchahtml = ('<div class="captcha-group clearfix"><div class="form-group pull-left"><img src="%s" alt="CAPTCHA code" width="162" height="77"></div><div class="form-group pull-right"><input type="text" class="form-control" placeholder="%s" name="captcha_code" required></div></div>').sprintf('./lib/simplecaptcha/loader.gif', $_LANG['captcha_code']);

		function af_get_captcha($form) {
			exec_ajax('member.getCaptcha', {}, function(status, data, xhr) {
				if (status === 'success') {
					$form.find('>.captcha-group img').attr('src', data['src']);
					return false;
