...
+function ($) {
  'use strict';
	...
	HTMLFormElement.prototype.dataExport = function() {
		var data = {}, arrs = $(this).serializeArray();
		for (var v in arrs) {
	...
	HTMLFormElement.prototype.dataImport = function(data) {
		...
		for (var key in data) {
			...
			if ($item.is("input")) {
				if($item.is(':file')) {
					...
					data[key] == 1 ? $item.attr('checked', 'checked') : $item.removeAttr('checked');
				} else if($item.is(':radio')) {
					$item.filter(function(){return this.value===data[key];}).attr('checked', 'checked');
				} else {
		...
		if($editor.length>0) {
			...
			if($uploads.length>0 && $.isArray(data['files']) && data['files'].length>0) {
				var html = '<div class="form-group has-feedback" style="margin-bottom:5px"><div class="af-editor-uploaded-list fileupload-group file-list form-control" style="margin-top:10px">';
