...
<?php
	...
	if(!$error) {
		...
		foreach ($file_list['data'] as $key => $value) {
			...
			echo '<tr class="afox-list-item" data-exec-ajax="admin.getFile" data-ajax-param="mf_srl,'.$value['mf_srl'].'" data-modal-target="#file_modal"><th scope="row" rowspan="2"><input type="radio" name="mf_standard" value="'.$value['mf_srl'].'" class="data_standard" style="margin-right:5px" except-exec-event><input type="checkbox" value="'.$value['mf_srl'].'" class="data_selecter" style="margin-right:5px" except-exec-event></th>';
			...
			echo '<tr class="afox-list-item" data-exec-ajax="admin.getFile" data-ajax-param="mf_srl,'.$value['mf_srl'].'" data-modal-target="#file_modal"><th scope="row"><a href="'.getUrl('category',$value['md_id']).'" except-exec-event>'.$value['md_id'].'</a></th>';
			echo '<td class="title"'.(empty($value['mf_size'])?' style="text-decoration:line-through"':'').'><input type="checkbox" value="'.$value['mf_srl'].'" class="data_selecter" style="display:none;margin-right:5px" except-exec-event>'.escapeHtml(cutstr($value['mf_name'],50)).'</td>';
