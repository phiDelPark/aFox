...
<section id="documentReply" aria-label="List of replies to this post">
	<?php
		...
		foreach ($REPLYS as $key => $value) {
			...
			echo '<a id="reply_'.$value['rp_srl'].'"'.(!empty($_POST['rp'])&&$value['rp_srl']==$_POST['rp'] ? ' class="active"':'').'></a>'
				...
				.'<div class="reply-date"><small>'.date('Y/m/d h:i', strtotime($value['rp_update'])).($rp_secret?'<svg class="bi me-1"><use href="'._AF_THEME_URL_.'bi-icons.svg#shield-lock"/></svg>':'').'</small></div></div></div><div class="right"><div class="content clearfix">'.toHTML($rp_content,$value['rp_type'],'current_replies').'</div>'
				.'<div class="area-button clearfix"><div class="pull-right">'
				.($is_rp_grant&&$_len<5&&$value['rp_status']!='4'?('<a href="#" data-exec-act="board.updateComment" data-act-param="rp_parent,'.$value['rp_srl'].'">'.getLang('reply').'</a>'):'')
				.'<a href="#" '.($value['grant_write']?'data-exec-act="board.getComment" data-act-param="rp_srl,'.$value['rp_srl'].'"'.(empty($value['mb_srl'])&&!$is_manager&&$rp_secret?' data-act-password="1"':''):$not_edit_str).'>'.getLang('edit').'</a>'
				.'<a href="#" '.($is_manager||$value['grant_write']?'data-exec-act="board.deleteComment" data-act-param="rp_srl,'.$value['rp_srl'].'"'.(empty($value['mb_srl'])&&!$is_manager?' data-act-password="1"':''):$not_edit_str).'>'.getLang('delete').'</a>'
				.'</div></div></div></div>';
		}
	?>
</section>
