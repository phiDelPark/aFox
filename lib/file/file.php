...
function setHttpError($e,$b=false){header('HTTP/1.1 '.$e); header("Connection: close");
if($b){set_error('HTTP/1.1 '.$e, 3); header('Location: '.$_SERVER['HTTP_REFERER']);}exit;}function file_triggerModuleCall($f,$_DATA) {
include $f; $r=null; $t='before_proc';if(function_exists($t)){$r=call_user_func($t, [$_DATA]);}
return $r===true ? $_DATA : false;}function file_triggerCall($rank, &$data) {
...
$ori_mdid = $out['md_id'];$ori_target = $out['mf_target'];if($out['mf_link']=='1'){
...
if(DB::error()) setHttpError('400 Bad Request');}$tmp=explode('/',$out['mf_type']);$ft=strtolower(array_shift($tmp));
...
if(empty($module['md_id'])) setHttpError('400 Bad Request', true);$_f[$key]['point'] = (int)$module['point_download'];
$_f[$key]['permission'] = !($_f[$key]['point'] < 0 && empty($mb_srl)); //포인트가 -면 비회원 다운 불가
$grant = $module['grant_download'];if($_f[$key]['permission']&&!empty($grant)){$tmp = ord($mb_rank);
$_f[$key]['permission'] = $tmp < 116 && ord($grant) <= $tmp; //0 = 48, z = 122 //s = 115 초과면 에러
if(!$_f[$key]['permission']) setHttpError('401 Unauthorized', true);
}} else if($_f[$key]['type']=='image'){if(!file_exists($_f[$key]['path'])){
$_f[$key]['path']=_AF_PATH_.'common/img/no_image.png';} elseif($thumb) {$size = $_GET['thumb'];
...
$tw = (int)$module['thumb_width'];$th = (int)$module['thumb_height'];$fit = $module['thumb_option']==='1';
} else { //사용자 썸네일 2가지 크기만
$size = explode('x', $size);$tw = $size[0]>100?200:($size[0]>0?100:0);$th = $size[1]>100?200:($size[1]>0?100:0);
$fit = $size[2]==='1';}if(!empty($tw)&&!empty($th)){
if(file_exists($tmp=_AF_ATTACH_DATA_.'thumbnail/'.$out['md_id'].'/'.$out['mf_target'].'/'.$out['mf_srl'].'_'.$tw.'x'.$th.'x'.($fit?'1':'0').'.png')){
$_f[$key]['path']=$tmp;} else {require_once dirname(__FILE__) . '/thumbnail.php';
...
$fstat=fstat($fp);if(!empty($_SERVER['HTTP_IF_MODIFIED_SINCE'])){
$tmp=strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']);if($tmp&&($tmp>=$fstat['mtime'])){fclose($fp); setHttpError('304 Not Modified');}}
if($_f[$key]['type']=='binary'){ //다운로드 조회 기록
$act='mf_download'; $point=$_f[$key]['point'];if(!empty($point)){$uinfo = ['mb_srl'=>$mb_srl,'ipaddress'=>$_SERVER['REMOTE_ADDR']];
$pkey = ($uinfo['mb_srl'] > 0 ? 'mb_srl':'mb_ipaddress');$pval = ($uinfo['mb_srl'] > 0 ? $uinfo['mb_srl']:$uinfo['ipaddress']);
$uinfo['data'] = DB::get(_AF_HISTORY_TABLE_,['hs_action'=>$act.'('.$srl.')',$pkey=>$pval]);
if(!empty($mb_srl) && !DB::error()){if(empty($uinfo['data'])){ //처음 한번만 //자신은 포인트 사용안함
if($mb_srl !== $_f[$key]['member']){$mb = DB::get(_AF_MEMBER_TABLE_,'mb_point',['mb_srl'=>$mb_srl]);
...
}} else setHttpError('500 Internal Server Error', true);} DB::update(_AF_FILE_TABLE_, ['^mf_download'=>'mf_download+1'], ['mf_srl'=>$srl]);}
header('Last-Modified: '.$fstat['mtime']);
header("Content-Disposition: attachment; filename=".$_f[$key]['name']);
header('Cache-Control:');header('Content-Type: '.$_f[$key]['mime']);
header("Connection: close");fpassthru($fp);fclose($fp);
