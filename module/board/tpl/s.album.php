...
<article class="clearfix">
	...
	<?php
		...
		foreach ($_{'board'}['data'] as $key => $val) {
			...
			$box_items[__MOBILE__?true:!$toggle] = '<a href="'.(!$wr_permit&&$wr_secret?'#" data-srl="'.$val['wr_srl'].'" data-toggle="modal" data-param="srl,'.$val['wr_srl'].'" data-target="#passwordBoxModal':getUrl('srl',$val['wr_srl'],'disp','','cpage','','rp','')).'"><div class="img-container" style="background-image: url(\''.(empty($_image['mf_srl'])?$_tmp_image:_AF_URL_.'?file='.$_image['mf_srl'].'&thumb').'\')"></div></a>';
			$box_items[__MOBILE__?false:$toggle] = '<h3 class="text-ellipsis"><a href="'.(!$wr_permit&&$wr_secret?'#" data-srl="'.$val['wr_srl'].'" data-toggle="modal" data-param="srl,'.$val['wr_srl'].'" data-target="#passwordBoxModal':getUrl('srl',$val['wr_srl'],'disp','','cpage','','rp','')).'">'.($wr_secret?$_tmp:'').escapeHtml($val['wr_title'], true).'</a>'.($val['wr_reply']>0?' <small>(+'.$val['wr_reply'].')</small>':'').'</h3>';
			$box_items[__MOBILE__?false:$toggle] .= '<div class="author clearfix"><span class="mb_nick pull-left" data-srl="'.$val['mb_srl'].'" data-rank="'.(ord($val['mb_rank']) - 48).'">'.$val['mb_nick'].'</span><span class="pull-right">'.date('Y/m/d', strtotime($val['wr_update'])).'</span></div>'.$wr_extra_vars.'<div class="wr_content">'.cutstr(!$wr_permit&&$wr_secret?getLang('msg_is_secret'):escapeHtml(toHTML(0,$val['wr_content']),true,ENT_QUOTES), __MOBILE__?100:300).'</div>';
			?>
