	...
	<?php
		...
		foreach ($_{'board'}['data'] as $key => $val) {
			...
			<li>
				...
				<div class="tmlabel">
					<h3 class="text-ellipsis"><?php echo '<a href="'.(!$wr_permit&&$wr_secret?'#" data-srl="'.$val['wr_srl'].'" data-toggle="modal" data-param="srl,'.$val['wr_srl'].'" data-target="#passwordBoxModal':getUrl('srl',$val['wr_srl'],'disp','','cpage','','rp','')).'"'.($val['wr_srl']==$srl?' class="active"':'').'>'.($wr_secret?$_tmp:'').escapeHtml($val['wr_title'], true).'</a>' ?></h3>
					<p><?php echo $wr_extra_vars.'<div class="wr_content">'.cutstr(!$wr_permit&&$wr_secret?getLang('msg_is_secret'):escapeHtml(toHTML(0,$val['wr_content']),true,ENT_QUOTES), __MOBILE__?100:300).($val['wr_reply']>0?' <small>(+'.$val['wr_reply'].')</small>':'').'</div>' ?></p>
				</div>
			</li>
			<?php
		}
