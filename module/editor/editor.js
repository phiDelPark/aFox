...
function afoxEditor(ID, OPTIONS) {
  ...
  const editor = document.querySelector("#editor" + ID.toUcFirst()),
    iframe = document.createElement("iframe"),
    textarea = editor.querySelector("[name=" + ID + "]");

  const typebar = editor.querySelectorAll("#editorTypebar [data-target]"),
  ...
  this.pasteHtml = (html) => {
    ...
    if (this.htmlMode) {
      ...
      const txtBefore = textarea.value.slice(0, selection.start),
  ...
  iframe.addEventListener("load", (ee) => {
    ...
    iframe.contentWindow.document.body.addEventListener("focus", (e) =>
      iframe.classList.add("focused")
    );
    iframe.contentWindow.document.body.addEventListener("blur", (e) =>
      iframe.classList.remove("focused", "is-invalid")
    );
    iframe.contentWindow.document.body.addEventListener("keydown", (e) => {
      if (this.htmlMode && e.key === "Tab") {
        e.preventDefault();
        const selection = this.getSelection();
        let range = selection.getRangeAt(0);
        if (range) {
          ...
          range.deleteContents();
          const regex = /(^|\n)(\t|\s{4})/g;
          div.innerHTML = e.shiftKey
            ? div.innerText.replaceAll(regex, "$1")
            : "\t" + div.innerText.replaceAll("\n", "\n\t");
          const el =
            is_pre || parent.style.whiteSpace == "pre"
              ? iframe.contentWindow.document.createTextNode(div.innerText)
              : div;
          range.insertNode(el);
  ...
  const clickRemoveFile = (e) => {
    ...
    const el_del = editor.querySelector(
      '[name="remove_files[]"][value="' + srl + '"]'
    );
    if (el_del) {
      el_del.remove();
  ...
  const dragStartFile = (e) => {
    ...
    html =
      a[2][1].slice(0, 5).toLowerCase() == "image"
  ...
  const changeUploadFile = (e) => {
    window.dispatchEvent(
      new Event("beforeunload", { bubbles: true, cancelable: false })
    );
    ...
    Object.keys(e.target.files).map((key) => {
      ...
      span.innerHTML =
        "<img src" +
        html +
        (image ? "" : ' srcset="./module/editor/bi-binary.svg"') +
        ">" +
  ...
  const clickToolbar = (e) => {
    ...
    const htmlToCmd = {
      ...
      insertorderedlist: this.htmlMode
        ? "<ol><li>%s</li></ol>"
        : "\n" + "1. " + "%s" + "\n" + "2. ",
      ...
      table: this.htmlMode
        ? "<table><tr><th>...</th><th>...</th></tr><tr><td>%s</td><td> </td></tr></table>"
        : "| ... | ... |\n|---|---|\n| %s |   |",
      codeblock: this.htmlMode
        ? "<pre><code>%s</code></pre>"
        : "```" + "\n" + "%s" + "\n" + "```",
    };
  ...
  const clickTypebar = (e) => {
    ...
    const elt = editor.querySelector("[name=" + e.target.dataset.target + "]"),
  ...
  chk_group.forEach((el) =>
    el.addEventListener("change", (_) => el.classList.remove("is-invalid"))
  );
